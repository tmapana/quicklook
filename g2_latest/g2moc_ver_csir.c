...
//  }
  ...
  ProcAveGPSHgt = 0.0;
  fprintf(LogFile,
    "INTLat (deg)\tINTLong (deg)\tINTHgt (m)\tLBR PRI\tGPS UTC (hh:min:sec)\n");
  ...
  ********************************************************************/
  ...
  ConvToCartes(ProcMotDataEnd.InertLat * TO_RAD,
               ProcMotDataEnd.InertLong * TO_RAD,
               ProcMotDataStart.INTHgt, /* use same height as start for calc */
               &RefEndPt);
  ConvToCartes(ProcMotDataStart.InertLat * TO_RAD,
               ProcMotDataStart.InertLong * TO_RAD,
               ProcMotDataStart.INTHgt,
               &RefStartPt);         
...
*/
  ...
  for (MotRec=0; MotRec< ProcNumMotRecords-1; MotRec++) {
    ...
    ConvToCartes(IMUDataNext.InertLat * TO_RAD,
                 IMUDataNext.InertLong * TO_RAD,
                 IMUDataNext.INTHgt,
                 &AircraftPosn);  
